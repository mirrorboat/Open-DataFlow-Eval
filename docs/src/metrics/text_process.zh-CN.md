...
此外，启发式规则过滤在预训练数据的筛选方面占有很大的比重，在这一方面，Dingo数据质量评估工具对我们的开发带来了很大的启发。我们在`dataflow/process/text/filters/heuristics.py`中整合了部分Dingo中使用的规则过滤算法，共22种。详见某个链接（Dingo还没有开源，先空在这里）。
