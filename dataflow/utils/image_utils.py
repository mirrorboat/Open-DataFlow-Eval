...
def pyiqa_image_preprocess(image):
    # resize, to_tensor
    return TF.to_tensor(image.resize((512, 512)).convert("RGB"))
...
def download_hf_model(model_cache_path, hf_model_path):
    command = ['huggingface-cli', 'download', '--resume-download', hf_model_path, '--local-dir', model_cache_path]
    print(f"Downloading {hf_model_path} to {model_cache_path}")
    result = subprocess.run(command, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Failed to download model: {hf_model_path}")
        print(result.stderr)
    else:
